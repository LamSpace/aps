...
**Choice:** Pre-compute MethodHandles at class initialization time via `Lookup.findSpecial(targetClass, methodName, methodType, generatedClass)`.

**Alternatives considered:**
- **`MethodHandle.bindTo(proxy).invoke(args)`:** Requires the target instance at bind time, not possible at class init.
- **`Lookup.unreflectSpecial(method, generatedClass)`:** Similar to `findSpecial` but adds an extra `Method` → `MethodHandle` conversion step.

**Rationale:** `findSpecial` directly produces the MethodHandle needed for `invokespecial` semantics, and doing it in `<clinit>` means the cost is paid once per class, not once per call.
