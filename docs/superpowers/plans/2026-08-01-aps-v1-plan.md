...
- Package: `io.github.lamspace` for public API; `io.github.lamspace.generator`, `io.github.lamspace.loader`, `io.github.lamspace.internal` for internals
...
mvn test -pl . -Dtest=MethodDispatcherTest -q
```
Expected: tests pass (bytecode structurally valid even without <clinit> generation yet, or may need a stub `ClassGenerator.generateClinitStatic` — see Task 6)
...
private void generateConstructor(ClassWriter cw, String generatedInternal,
    ...
    for (int i = 0; i < constructorArgs.length; i++) {
        ...
        if (argType == Integer.class || argType == int.class) {
            mv.visitVarInsn(Opcodes.ILOAD, slot); slot++;
        } else if (argType == Long.class || argType == long.class) {
            mv.visitVarInsn(Opcodes.LLOAD, slot); slot += 2;
        } else if (argType == Float.class || argType == float.class) {
            mv.visitVarInsn(Opcodes.FLOAD, slot); slot++;
        } else if (argType == Double.class || argType == double.class) {
            mv.visitVarInsn(Opcodes.DLOAD, slot); slot += 2;
            ...
            mv.visitVarInsn(Opcodes.ALOAD, slot); slot++;
        }
    }
