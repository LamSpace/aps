...
**Goal:** Build APS Core — a MethodHandle-powered dynamic proxy library capable of proxying classes with performance
exceeding CGLib.

**Architecture:** Three-layer design: Public API (`APS` with static factories), Bytecode Engine (ASM subclass
generation + MethodHandle super-call binding), and Class Definition (`Lookup.defineHiddenClass` wrapper). Single
Callback model matching CGLib/Proxy semantics.
...
- Produces: `MethodDispatcher.dispatchMethods(ClassWriter, Class<?>, String, ClassFilter): List<String>` — generates
  method override bytecode bodies, static fields for Method/MethodHandle, and clinit registry entries; returns
...
- Produces:
    - `APS.create(Class<T>, Callback): T`
    - `APS.create(Class<T>, Callback, ClassFilter): T`
...
- Extends: `APS.create(Class<T>, Callback, ClassFilter, Object...): T` accepts constructor args
...
- [ ] **Step 4: Add APS.create overload with constructor args**
...
- No new public API — benchmarks exercise existing `APS.create()`
